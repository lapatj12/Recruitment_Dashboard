...
function renderOverview() {
  ...
  charts.company = new Chart(document.getElementById('chartCompany'), {
    type: 'doughnut',
    data: { labels: ['Manpower','HR Digest'], datasets: [{ data: [manpower, hrd], backgroundColor: [CHART_COLORS.teal, CHART_COLORS.violet], borderWidth: 0 }] },
    options: {
...
function renderRecruitment() {
  ...
  charts.leadTime = new Chart(document.getElementById('chartLeadTime'), {
    ...
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, datalabels: smartBarLabels({ formatter: v => v + 'd' }) },
...
/** SLA = 14 days from request to confirmed. Compares On-KPI (<=14d) vs
 * Over-KPI (>14d) counts per service provider, plus the %Over-KPI rate. */
function renderSlaBreakdown(data) {
  const SLA_DAYS = 14;
  const bySla = {};
  data.forEach(r => {
    const d = Number(r.diff_days);
    if (isNaN(d)) return;
    const c = r.company;
  ...
  charts.reasonsByCompany = new Chart(document.getElementById('chartReasonsByCompany'), {
    ...
    data: {
      ...
      datasets: companies.map((c, i) => ({
        ...
        backgroundColor: i === 0 ? CHART_COLORS.teal : CHART_COLORS.violet,
        borderRadius: 5,
      }))
    },
    options: {
